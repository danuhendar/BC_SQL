...
public class Global_function {
    ...
    public Global_function(Boolean include_connection){
    	//this.inter_login=inter_login;
    	inter_login = Entity.getInter_login();
    	Boolean res_cek_koneksi = Entity.getInter_login().cek_koneksi_db();
    	System.out.println("res_cek_koneksi global : "+res_cek_koneksi);
    }
   ...
   public boolean is_proses_setting_main() {
       	...
       	try {
            ...
            mqtt_client.subscribe(rtopic_command,qos_message,new IMqttMessageListener() {
                ...
                if(IN_SOURCE.equals("IDMCommandListeners")){
                    ...
                    try{
                       try {
                    	   ...
                    	   String get_kdtk[] = GetTransReport("SELECT TOKO,STATION,KDCAB,NAMA FROM tokomain where IP = '"+res_in_to+"'", 4, false).split("~")[0].split("%");
                           kdtk = get_kdtk[0];
                           IN_STATION = get_kdtk[1];
                           res_in_kdcab = get_kdtk[2];
                           nm_pc = get_kdtk[3];
                    ...
                    {
                    	 if(res_in_to.equals("IDMCommandListeners")) {
                    		 kdtk = "";
                             nm_pc = "";
                    	 }else {
                    		 try {
                        		 //System.out.println("SELECT TOKO,STATION,KDCAB,NAMA FROM tokomain where IP = '"+res_in_to+"'");
                        		 String get_kdtk[] = GetTransReport("SELECT TOKO,STATION,KDCAB,NAMA FROM tokomain where IP = '"+res_in_to+"'", 4, false).split("~")[0].split("%");
                                 kdtk = get_kdtk[0];
                                 IN_STATION = get_kdtk[1];
                                 res_in_kdcab = get_kdtk[2];
                            	 ...
                            	 String get_kdtk[] = GetTransReport("SELECT TOKO,STATION,KDCAB,NAMA FROM tokomain where IP = '"+res_in_from+"'", 4, false).split("~")[0].split("%");
                                 kdtk = get_kdtk[0];
                                 IN_STATION = get_kdtk[1];
                                 res_in_kdcab = get_kdtk[2];
                                 nm_pc = get_kdtk[3];
...
//            if(Boolean.parseBoolean(Entity.getTampilkan_query_console())){
//                System.err.println("query_transreport : "+query);
//            }else{
//                
//            }
            
            if(Entity.getInter_login().cek_koneksi_db()){
            ...
            if(kdtk.equals("") || kdtk.equals("-")) {
          	  
            }else {
            	//WriteLog("query",query, true);
...
//        fileHandler.close();
    ...
    public String get_tanggal_curdate(){
     ...
     public void WriteLog(String rnama_file,String content_after,boolean append_content){
        ...
        try {
            ...
            String nama_file = rnama_file+"_"+tanggal+".txt";
...
/* Decryption Method */
    ...
    public MqttClient get_ConnectionMQtt(int profil) {
			...
			Boolean res_cleansession = false;
		   	...
		   	String ip_broker_primary = "";
		   	int port_broker_primary = 0;
		   	String username_broker_primary = "";
		   	String password_broker_primary = "";
		   	
		        try {
		        	
		        	if(profil == 1) {
		        		String cleansession = en.getCleansession_primary();
		        		res_cleansession = Boolean.parseBoolean(cleansession);
		        		
		        		String keepalive = en.getKeepalive_primary();
		        		res_keepalive = 60;//Integer.parseInt(keepalive);
		        		
		        		String reconnect = en.getReconnect_primary();
		        		res_reconnect = Boolean.parseBoolean(reconnect);
		        		
		        		String will_retained =  en.getWill_retained_primary();
		        		res_will_retained = Boolean.parseBoolean(will_retained);
		        		
		        		port_broker_primary = Integer.parseInt(en.getPort_broker_primary());
		        		
		        		ip_broker_primary = en.getIp_broker_primary();
		        		
		        		username_broker_primary = en.getUsername_broker_primary();
		        		
		        		password_broker_primary = en.getPassword_broker_primary();
		        		
		        	}else {
		        		String cleansession = en.getCleansession_secondary();
		        		res_cleansession = Boolean.parseBoolean(cleansession);
		        		
		        		String keepalive = en.getKeepalive_secondary();
		        		res_keepalive = Integer.parseInt(keepalive);
		        		
		        		String reconnect = en.getReconnect_secondary();
		        		res_reconnect = Boolean.parseBoolean(reconnect);
		        		
		        		String will_retained =  en.getWill_retained_secondary();
		        		res_will_retained = Boolean.parseBoolean(will_retained);
		        		
		        		port_broker_primary = Integer.parseInt(en.getPort_broker_secondary());
		        		
		        		ip_broker_primary = en.getIp_broker_secondary();
		        		
		        		username_broker_primary = en.getUsername_broker_secondary();
		        		
		        		password_broker_primary = en.getPassword_broker_secondary();
		        	}
		        	
		            
		            System.out.println("res_cleansession	: "+res_cleansession);
		            System.out.println("res_keepalive		: "+res_cleansession);
		            System.out.println("res_reconnect		: "+res_reconnect);
		            System.out.println("username_broker_primary		: "+username_broker_primary);
		            System.out.println("password_broker_primary		: "+password_broker_primary);
		            ...
		            //-- sesi koneksi db --//
		            String res_broker       = ip_broker_primary+":"+port_broker_primary;
		            /* ssl://mqtt.cumulocity.com:8883 for a secure connection */
		            //-------------------------------- TRANS CONNECTION ----------------------//
		            
		                final String serverUrl   = "tcp://"+res_broker;
		                System.out.println("serverUrl "+profil+" : "+serverUrl);
		                
		                String clientId = UUID.randomUUID().toString();
		                MemoryPersistence persistence = new MemoryPersistence();
		                try {
		                	 
		                	mqtt_client = new MqttClient(serverUrl, clientId,persistence);
		                	
					} catch (MqttException e) {
						// TODO Auto-generated catch block
						System.err.println("Create Object Mqtt Client error : "+e.getMessage());
					}
		                MqttConnectOptions options = new MqttConnectOptions();
		                options.setCleanSession(res_cleansession);
		             
		                options.setKeepAliveInterval(res_keepalive);
		                options.setAutomaticReconnect(res_reconnect);
		                
		                if(username_broker_primary.equals("null")||password_broker_primary.equals("null")){
		                    
		                }else if(username_broker_primary.equals("")||password_broker_primary.equals("")) {
		                
		                }else{
		                    options.setUserName(username_broker_primary);
		                    options.setPassword(password_broker_primary.toCharArray());
		                }
		                
		                try {
		                	mqtt_client.connect(options); 
		                }catch(Exception exc) {
		                	System.err.println("Error konek ke broker : "+exc.getMessage().toString());
		                }
		               
		                
		                System.out.println("Konek ke Broker "+profil);
    ...
    public String getTampilkan_query_consoleSetting() {
    	String res = "";
    	try {
    		res = en.getTampilkan_query_console();
    	}catch(Exception exc) {}
    	
    	return res;
    }
    
    public String getCabangSetting() {
    	String res = "";
    	try {
    		res = en.getCabang();
    	}catch(Exception exc) {}
